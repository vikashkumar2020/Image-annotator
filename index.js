...
(() => {
    ...
    // Parameters
    const saveInterval = 30 // Bbox recovery save in seconds **
    const fontBaseSize = 30 // Text size in pixels
    const fontColor = "rgb(124,173,91)" // Base font color
    const borderColor = "rgb(124,173,91)" // Base bbox border color
    const backgroundColor = "rgba(0, 116, 217,0.5)" // Base bbox fill color
    const markedFontColor = "#FF4136" // Marked bbox font color
    const markedBorderColor = "rgb(194,38,24)" // Marked bbox border color
    ...
    //image extensions
    const extensions = ["jpg", "jpeg", "png", "JPG", "JPEG", "PNG", "gif"]
    ...
    //To check local storage support in browser
    const isSupported = () => {
        try {
    ...
    // Save bounding boxes to local storage every X seconds
    if (isSupported() === true) {
        setInterval(() => {
    ...
    // Start everything
    //onreadystatechange https://developer.mozilla.org/en-US/docs/Web/API/Document/readyState
    ...
    //draw image on screen using canvas or display image  
    const drawImage = (context) => {
        //set image coordinate to zoom till current image size
        //zoomX(0) and zoomY(0) setting origin of image
    ...
    function download() {
        ...
        hidden_canv.width = currentImage.width;
        hidden_canv.height = currentImage.height;

        //Draw the data you want to download to the hidden canvas
        var hidden_ctx = hidden_canv.getContext('2d');
        var img = new Image();
        canvas.renderCanvasElements();
        ...
        hidden_ctx.drawImage(
            ...
            currentImage.height,//Clipping Height
            0,//Place X
            0,//Place Y
    ...
    //drawIntro not work if comment down
    ...
    const drawNewBbox = (context) => {
        if (mouse.buttonL === true && currentClass !== null && currentBbox === null) {
            //destructuring in js
            const { width, height } = newFunction()
    ...
    //for setting + sign between bboxes
    ...
    //move pointer only inside image for this cross must draw inside and
    //bboxes would draw inside image
    const inside_image = (x, y, width, height, x_, y_) => {
        return x_ >= x && x_ <= x + width && y_ >= y && y_ <= y + height
    }

    //styles of bboxes
    const setBBoxStyles = (context, marked) => {
        // initialise with empty array so that no dash appear in 
        //Bounding box
        context.setLineDash([])
    ...
    const setBboxCoordinates = (x, y, width, height) => {
        ...
        document.getElementById("bboxInformation").innerHTML = `Bounding Box Dimension ${width}x${height} (${x}, ${y}) (${x2}, ${y2})`
    }
        ...
        if (event.type === "mousedown" && inside_image(0, 0, currentImage.width, currentImage.height, mouse.realX, mouse.realY) === true) {
           ...
           // https://stackoverflow.com/questions/56199283/how-to-catch-mouse-left-button-on-mouseover-event
            ...
            if (mouse.buttonL === true && currentImage !== null && currentClass !== null && inside_image(0, 0, currentImage.width, currentImage.height, mouse.realX, mouse.realY) === true) {
                ...
                if (movedWidth > minBBoxWidth && movedHeight > minBBoxHeight && inside_image(0, 0, currentImage.width, currentImage.height, mouse.realX, mouse.realY) === true) { // Only add if bbox is big enough
                    if (currentBbox === null) { // Add only when no other bbox is selected
    ...
    //zoom function to zoom the image 
    const zoom = (number) => {
    ...
    const setCurrentImage = (image) => {
        ...
        reader.addEventListener("load", () => {
            ...
            document.getElementById("imageInformation")
                .innerHTML = `Selected Image dimension, Image size : ${image.width}x${image.height}, ${formatBytes(image.meta.size)}`
        })
    ...
    const listenBboxCocoSave = () => {
        document.getElementById("saveCocoBboxes").addEventListener("click", () => {
            ...
            for (let imageName in bboxes) {
                ...
                zip.file("bbox.json", JSON.stringify(result))
            }

            zip.generateAsync({ type: "blob" })
                .then((blob) => {
                    saveAs(blob, "bboxes_json.zip")
                })
        })
    }
