(function (window) {
  !(function (e) {
    ...
    var n,
      t = 0,
      a = ["moz", "webkit", "o", "ms"];
    for (n = 0; n < a.length && !e.requestAnimationFrame; n += 1)
      (e.requestAnimationFrame = e[a[n] + "RequestAnimationFrame"]),
  ...
  window.Canvas = class Canvas {
    constructor() {
      ...
      this.renderer = this.element.ctx = this.element.getContext(
        arguments[3] || "2d"
      );
      ...
      if (
        arguments[1] == window.innerWidth &&
        arguments[2] == window.innerHeight
      )
    ...
    fireEvent(event) {
      ...
      if (this.eventListeners.has(event))
        this.eventListeners.get(event).apply(this, args);
    }
    drawWorkingFrame() {
    ...
    _loop() {
      ...
      this._lastFrame = now;
      if (!this._paused)
        this.fireEvent("update", this._HANDYOBJECT_, delta, Canvas.now());
    ...
    loadObject(key) {
      key = key || "HO";
      const handyObject = JSON.parse(
        window.localStorage.getItem("canvas-" + key)
      );
      if (!handyObject || typeof handyObject !== "object") return !1;
    ...
    renderCanvasElements() {
      ...
      for (let element of this.canvasElements) {
        if (element.animation) {
          const ratio =
            (Canvas.now() - element.animation.start) /
            (element.animation.end - element.animation.start);
          if (ratio >= 1) {
            element.animation = !1;
          } else {
            switch (element.animation.type) {
              case "move":
                element.x = Canvas.lerp(
                  element.animation.from.x,
                  element.animation.to.x,
                  ratio
                );
                element.y = Canvas.lerp(
                  element.animation.from.y,
                  element.animation.to.y,
                  ratio
                );
                break;
              case "lineLength":
                element.lineLength = Canvas.lerp(
                  element.animation.from.lineLength,
                  element.animation.to.lineLength,
                  ratio
                );
            }
          }
        ...
        switch (element.constructor.name) {
          ...
          case "CanvasLine":
            ...
            renderer.lineTo(
              Canvas.lerp(element.from.x, element.to.x, element.lineLength),
              Canvas.lerp(element.from.y, element.to.y, element.lineLength)
            );
  ...
  Canvas.Element = class Element {
    constructor() {
      ...
      this.rotation =
        typeof args.rotation !== "undefined" ? args.rotation : null;
      this.animation = !1;
    }
    animate() {
      ...
      switch (type) {
        case "move":
          this.animation = {
            ...
            to: { x: args.x, y: args.y },
          };
          break;
        case "lineLength":
          this.animation = {
            type: type,
            from: { lineLength: this.lineLength },
